...
fn candidate_dirs() -> Vec<PathBuf> {
    ...
    let mut dirs: Vec<PathBuf> = std::iter::once(PathBuf::from("/Library/Java/JavaVirtualMachines"))
...
fn jdk_from_standard(root: &Path, name: &str) -> Option<Jdk> {
    ...
    Some(Jdk { name: name.to_string(), version, path: home, vendor: infer_vendor(name) })
}
...
fn major_version(full: &str) -> String {
    ...
    parts.first().map(|s| s.to_string()).unwrap_or_else(|| full.to_string())
}
...
mod tests {
    ...
    fn jdk_matches_by_major() {
        let jdk = Jdk { name: "test".into(), version: "17".into(), path: PathBuf::from("/x"), vendor: "Test".into() };
