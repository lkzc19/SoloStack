...
pub fn list_log_dates() -> Result<Vec<String>, String> {
    let mut dates: Vec<String> = list_log_files()?.into_iter().filter_map(|p| {
        p.file_name().and_then(|n| n.to_str()).and_then(log_date_of)
    }).collect();
...
pub fn read_logs_for(date: &str) -> Result<String, String> {
    ...
    let path = if file.is_file() {
        ...
        if legacy.is_file() { legacy } else { file }
    };
    if !path.is_file() {
...
/// 供测试等判断某日期文件路径。
pub fn log_file_exists(date: &str) -> bool {
    log_file_for(date).map(|p| p.is_file()).unwrap_or(false)
}

/// 读日志文件（供其它组件如命令行按路径 tail；此处为保持日志目录只读接口保留一个便利方法）。
...
mod tests {
    ...
    fn log_date_parses_both_formats() {
        assert_eq!(log_date_of("solostack.log.2026-09-05").as_deref(), Some("2026-09-05"));
        assert_eq!(log_date_of("solostack-2026-09-04.log").as_deref(), Some("2026-09-04"));
