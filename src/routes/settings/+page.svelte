    ...
    {:else if settingsTab === "environments"}
      <section class="settings-section">
        ...
        <p class="settings-desc">
          查看环境及组件概要；可以切换、重命名或删除非活动环境。
        </p>

        <div class="environment-list">
          {#each store.environments as environment (environment.id)}
            <div class="environment-row" class:active={environment.active}>
              <span class="environment-icon"><Layers3 size={16} /></span>
              <div class="environment-copy">
                {#if editingEnvironmentId === environment.id}
                  <div class="environment-rename-row">
                    <input
                      class="input"
                ...
                <div class="environment-components">
                  {#if environment.components.length === 0}
                    <span class="environment-empty">暂无组件</span>
                  {:else}
                    {#each environment.components as item (item.component)}
                      <span class="environment-component">
                        {componentAdapter(item.component)?.displayName ?? item.component}
                        v{item.version}
                      </span>
                    {/each}
                  {/if}
                </div>
                <div class="environment-dates">
                  <span>创建于 {formatEnvironmentTime(environment.created_at)}</span>
                  <span>最近激活 {formatEnvironmentTime(environment.last_activated_at)}</span>
                </div>
              </div>
