...
- 组件元数据以 JSON 模板描述，包含：架构适配建议、推荐版本、官方下载地址、SHA256、启停脚本路径、配置文件路径、日志目录、默认端口、依赖 JDK 版本。
...
- 后端核心：**Rust**（进程管理、文件处理、下载解压、日志监听、配置解析）。采用 workspace 结构：`src-tauri`（GUI）+ `crates/core`（核心逻辑，GUI 与 CLI 共享）+ `crates/cli`（命令行工具，与 GUI 平级交付）。
...
~/.solostack/
├─ jdk/                 # 托管的各版本 JDK
├─ zk/                  # Zookeeper 多版本（V1.1 起）
├─ kafka/               # Kafka 多版本（V1.1 起）
├─ flink/               # Flink 多版本（V1.2 起）
├─ hadoop/              # Hadoop 多版本（V1.0 起）
├─ .templates/          # 组件元数据 JSON 模板（可扩展）
...
```jsonc
{
  ...
  "version": "4.x",
  "downloadUrl": "https://...",     // 官方下载地址（或用户本地路径）
  "sha256": "...",                  // 安装包校验值
