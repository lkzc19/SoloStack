...
- **本机 JDK 扫描**：扫描 macOS 惯例的三个 JDK 位置——`/Library/Java/JavaVirtualMachines/`（系统级）、`~/Library/Java/JavaVirtualMachines/`（用户级）、`/opt/homebrew/opt/`（brew 安装的 openjdk）。从各 JDK 的 `release` 文件读取版本号（主版本）。
- **手动指定 + 自动匹配**：扫描结果在 GUI 展示，用户可手动指定任意一个；组件安装时若无本地托管 JDK，自动查找本机已装 JDK 中满足组件 `jdkRequirement` 的版本，有现成的则直接使用、避免重复下载。
...
- **内置源**：
  - **清华 TUNA 镜像（默认第一优先）**：`mirrors.tuna.tsinghua.edu.cn`，国内访问快、同步及时。
  - **官方源（备选）**：Apache / 各组件官方站。
...
- **默认清华**：首次使用默认清华镜像（国内访问更快），用户可按需切换到官方源或自定义源。
- **镜像访问兼容**：下载时使用浏览器 UA 头，避免国内镜像反爬策略（如 nginx）拒绝非浏览器 UA。
