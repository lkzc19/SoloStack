...
- 版本：V1.0 MVP（修订版 Rev.5）
- 适配平台：macOS（产品目标优先 Apple Silicon M 系列；当前开发机为 Intel Mac，MVP 在 x86_64 上验证）
...
原名 DevNest、CanoeStack，现定名 **SoloStack**。产品定义与定位不变。SoloStack 是一款 macOS 桌面端大数据组件本地管理器，让开发者**在本地一键恢复一套可运行的大数据测试环境**：从下载、配置、启停、看日志到卸载，全部可视化完成，彻底摆脱"上云才能做功能性测试"的依赖。核心目标是解决**本地环境可复现**与**操作零门槛**两个问题。
...
- **Mac 架构感知**：自动识别 ARM64 / x86_64，在下载、校验、运行前给出架构适配提示，并指引用户使用对应架构的官方包。

### 2.3 目标用户与验证基准

- **目标用户**：macOS 平台的大数据开发工程师、大数据学习者：日常需要在本机测试 Hadoop、Kafka、Flink、Zookeeper 组件，受限于 Mac 适配、命令行繁琐、环境不可复刻等问题。
- **开发机与验证基准**：当前开发机为 Intel Mac，MVP 阶段在 x86_64 上完成开发与自测；产品目标优先 **Apple Silicon（M 系列）**，架构感知逻辑在 core 层统一实现，Intel 与 ARM 共用同一套下载 / 校验 / 运行流程，仅选包与运行参数存在差异。
...
- 自动识别当前 Mac 架构（ARM64 / x86_64），下载前给出架构匹配校验与提示。
...
- **Hadoop 3.5.0 单机（伪分布式）** 跑通最小闭环：注册 → 下载/校验 → 配置 → 启停 → 日志 → 卸载 → 快照。
  - 当前开发机为 Intel Mac：下载官方 x86_64 包验证；Apple Silicon 上使用官方 3.4+ aarch64 包（已在官方支持范围内）。
...
- 后端核心：**Rust**（进程管理、文件处理、下载解压、日志监听、配置解析）。采用 workspace 结构：`src-tauri`（GUI）+ `crates/core`（核心逻辑，GUI 与 CLI 共享）+ `crates/cli`（命令行工具）。
