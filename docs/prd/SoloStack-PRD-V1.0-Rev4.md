...
- 版本：V1.0 MVP（修订版 Rev.4）
- 适配平台：macOS 专属（优先 Apple Silicon M 系列，兼容 Intel Mac）
...
macOS 平台的大数据开发工程师、大数据学习者：日常需要在本机测试 Hadoop、Kafka、Flink、Zookeeper 组件，受限于 Mac 适配、命令行繁琐、环境不可复刻等问题。
...
- **Kafka（KRaft 单机模式）** 跑通最小闭环：注册 → 下载/校验 → 配置 → 启停 → 日志 → 卸载 → 快照。
...
- 为需要旧版本（3.1.x / 3.2.x）的用户提供"社区 ARM 适配包"的指引入口（仅指引，不自研编译）。
- HDFS 数据目录与快照导入的冲突处理。
...
- 后端核心：**Rust**（进程管理、文件处理、下载解压、日志监听、配置解析）。
- 前端：**Svelte**（编译型、产物轻、贴合轻量定位）+ 组件库待定（优先 shadcn-svelte / Skeleton UI）。
...
├─ kafka/               # Kafka 多版本
├─ flink/               # Flink 多版本（V1.1 起）
└─ hadoop/              # Hadoop 多版本（V1.2 起）
```
...
### 7.1 MVP 验收标准（以 Kafka KRaft 为例）

1. 首次使用：从注册到成功启动，全程 GUI 完成，无需打开终端。
2. 停止、重启、状态检测准确；退出应用后无残留 Kafka 进程。
3. 端口冲突 / JDK 缺失 / 配置非法时给出明确提示而非静默失败。
4. 日志实时滚动、可搜索、可导出。
5. 卸载后 `~/SoloStack/kafka/` 对应目录与进程、缓存完全清理。
