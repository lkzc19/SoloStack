...
│  ├─ lib/hadoop/hadoop-3.5.0/  # 持久数据（HDFS 存储，快照不迁移）
...
| `var/lib/` | `var/lib` | 持久运行时数据（HDFS 存储等） |
...
- **可变数据集中**：运行时数据统一收在 `var/` 下（`var/lib` 持久数据、`var/log` 日志、`var/run` 进程记录），与安装物（`components/`）分离——升级、重装、卸载组件都不影响已有数据。
- **数据与组件分离**：HDFS NameNode/DataNode 存储等持久数据放 `var/lib/`，快照默认不迁移数据，导入时提示重新初始化或格式化。
