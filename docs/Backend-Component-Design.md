...
| 日期 | 决策 |
|---|---|
| 2026-09-17 | **引入多环境模型**：环境使用稳定 8 位 NanoID 和用户可见名称，所有组件、配置、日志、PID 与数据按 `environment_id` 隔离；同一环境中的同一种组件仅允许一个版本，切换环境前必须停止并验证旧环境全部组件。安装包缓存改为 `cache/downloads/` 的应用级共享缓存。 |
